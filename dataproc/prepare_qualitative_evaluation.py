    ...
    with open("%s/qualitative_eval_full.md" % (MIMIC_3_DIR), "w") as of:
        with open("%s/qualitative_eval_full_key.md" % (MIMIC_3_DIR), "w") as kf:
                ...
                for idx, row in tqdm(enumerate(r)):
                    ...
                    for code in codes:
                        ...
                        if (
                            ...
                            and key in lr_keys
                            and key in sim_keys
                            and key in attn_keys
                            and code_counts[code] < MAX_CODE_OCCURRENCES
                        ):
                            if sim_vals[key] == 0:
                                continue
                            code_counts[code] += 1
            ...
            for i, (key, toks) in enumerate(valid_texts[:NUM_QUESTIONS]):
                ...
                for i, (method, window) in enumerate(
                    np.random.permutation(
                        [
                            ("attn", attn_windows[key]),
                            ("conv", conv_windows[key]),
                            ("lr", lr_windows[key]),
                            ("sim", sim_windows[key]),
                        ]
                    )
                    ...
                    post = toks[
                        window + filter_size : window + filter_size + (CONTEXT_SIZE / 2)
                    ]
                    md_out = (
                        " ".join(pre) + " **" + " ".join(mid) + "** " + " ".join(post)
                    )
                    of.write("%s) %s\n\n" % (opts[i], md_out))
                    kf.write("%s (%s) %s\n\n" % (opts[i], method, md_out))
            print(
                "percentage of valid document-code pairs: %f"
                % (len(valid_texts) / float(num_pairs))
            )
