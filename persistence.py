...
def save_metrics(metrics_hist_all, model_dir):
    with open(model_dir + "/metrics.json", "w") as metrics_file:
        ...
        data.update(
            {"%s_te" % (name): val for (name, val) in metrics_hist_all[1].items()}
        )
        data.update(
            {"%s_tr" % (name): val for (name, val) in metrics_hist_all[2].items()}
        )
...
def write_preds(yhat, model_dir, hids, fold, ind2c, yhat_raw=None):
    ...
    with open(preds_file, "w") as f:
        ...
        for yhat_, hid in zip(yhat, hids):
            codes = []
            for ind in np.nonzero(yhat_)[0]:
                codes.append(ind2c[str(ind)])
            if len(codes) == 0:
                w.writerow([hid, ""])
            else:
    ...
    if fold != "train" and yhat_raw is not None:
        ...
        for i, (top_idxs, hid) in enumerate(zip(sortd, hids)):
            scores[int(hid)] = {
                ind2c[str(idx)]: float(yhat_raw[i][idx]) for idx in top_idxs[:100]
            }
...
def save_everything(
    args, metrics_hist_all, model, model_dir, params, criterion, evaluate=False
):
    """
