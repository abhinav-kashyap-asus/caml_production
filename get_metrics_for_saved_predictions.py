...
if len(sys.argv) < 2:
    print(
        "usage: python get_metrics_for_saved_predictions.py [path_to_saved_predictions_dir]"
    )
    sys.exit(0)
...
train_file = (
    "%s/train.csv" % data_dir
    if version == "mimic2"
    else "%s/train_%s.csv" % (data_dir, str(Y))
)
test_file = (
    "%s/test.csv" % data_dir
    if version == "mimic2"
    else "%s/test_%s.csv" % (data_dir, str(Y))
)
...
if version == "mimic3" and Y == "full":
    ...
    (
        ...
        hadm_ids,
        type_dicts,
    ) = evaluation.results_by_type(Y, model_dir, version)
