...
def init(args):
    ...
    with open(args.vocab_dicts_file) as fp:
        dicts = json.load(fp)
    ...
    for epoch in range(args.n_epochs):
        ...
        if epoch == 0 and not args.test_model:
            model_dir = os.path.join(
                MODEL_DIR,
                "_".join(
                    [args.model, time.strftime("%b_%d_%H:%M:%S", time.localtime())]
                ),
            )
            os.mkdir(model_dir)
        ...
        # metrics_all[0] is for validation
        for name in metrics_all[0].keys():
            metrics_hist[name].append(metrics_all[0][name])

        # metrics_all[1] is for testing
        for name in metrics_all[1].keys():
            metrics_hist_te[name].append(metrics_all[1][name])

        # metrics_all[2] is for training
        ...
        persistence.save_everything(
            args, metrics_hist_all, model, model_dir, params, args.criterion, evaluate
        )
    ...
    if not np.all(np.isnan(metrics_hist[criterion])):
        if len(metrics_hist[criterion]) >= patience:
            if criterion == "loss_dev":
                return (
                        np.nanargmin(metrics_hist[criterion])
                        < len(metrics_hist[criterion]) - patience
                )
            else:
                return (
                        np.nanargmax(metrics_hist[criterion])
                        < len(metrics_hist[criterion]) - patience
                )
    else:
...
def train(
        model, optimizer, Y, epoch, batch_size, data_path, gpu, version, dicts, quiet
):
    """
    Training loop.
    ...
    model.train()
    gen = datasets.data_generator(
        data_path, dicts, batch_size, num_labels, version=version, desc_embed=desc_embed
    )
    for batch_idx, tup in tqdm(enumerate(gen)):
        ...
        if not quiet and batch_idx % print_every == 0:
            ...
            print(
                "Train epoch: {} [batch #{}, batch_size {}, seq length {}]\tLoss: {:.6f}".format(
                    epoch,
                    batch_idx,
                    data.size()[0],
                    data.size()[1],
...
def test(
    ...
    model.eval()
    gen = datasets.data_generator(
        filename, dicts, 1, num_labels, version=version, desc_embed=desc_embed
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="train a neural network on some clinical documents"
    )
    parser.add_argument(
        "data_path",
        type=str,
    ...
    parser.add_argument(
        "--vocab-dicts-file",
        dest="vocab_dicts_file",
        type=str,
        required=True,
        help="file holding all the vocab dictionaries for training"
    )
