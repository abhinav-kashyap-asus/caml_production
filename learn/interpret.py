...
def save_samples(
    data, output, target_data, s, filter_size, tp_file, fp_file, dicts=None
):
    """
...
def important_spans(
    ...
    for p_code in pred_codes:
        ...
        if output[0][p_code] > 0.5 and (fps ^ (p_code in tgt_codes)):
            ...
            prelude = "top three important windows for %s code %s (%s: %s)" % (
                typ,
                str(p_code),
                code,
                desc_dict[code],
            ...
            while len(kgram_strs) < 3 and i < len(windows):
                ...
                words = [
                    ind2w[w] if w in ind2w.keys() else "UNK"
                    for w in data[0][start:end].data.cpu().numpy()
                ]
...
def make_windows(starts, filter_size, attn):
    ...
    windows = []
    overlaps_w_next = [
        starts[i + 1] < starts[i] + filter_size for i in range(len(starts) - 1)
    ]
    ...
    window_scores = sorted(
        window_scores.items(), key=operator.itemgetter(1), reverse=True
    )
