...
def all_metrics(yhat, y, k=8, yhat_raw=None, calc_auc=True):
    ...
    if yhat_raw is not None and calc_auc:
        ...
        for k_i in k:
            ...
            metrics["f1_at_%d" % k_i] = (
                2 * (prec_at_k * rec_at_k) / (prec_at_k + rec_at_k)
            )
...
def all_macro(yhat, y):
    return (
        macro_accuracy(yhat, y),
        macro_precision(yhat, y),
        macro_recall(yhat, y),
        macro_f1(yhat, y),
    )
...
def all_micro(yhatmic, ymic):
    return (
        micro_accuracy(yhatmic, ymic),
        micro_precision(yhatmic, ymic),
        micro_recall(yhatmic, ymic),
...
def results_by_type(Y, mdir, version="mimic3"):
    ...
    test_file = (
        "%s/test_%s.csv" % (MIMIC_3_DIR, str(Y))
        if version == "mimic3"
        else "%s/test.csv" % MIMIC_2_DIR
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print(
            ...
            + str(
                ...
                + " [train_dataset] [|Y| (as string)] [version (mimic2 or mimic3)] [model_dir]"
            )
        )
        sys.exit(0)
    ...
    (
        ...
        hadm_ids,
        type_dicts,
    ) = results_by_type(Y, mdir, version)
