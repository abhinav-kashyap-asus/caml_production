...
def data_generator(
        filename, dicts, batch_size, num_labels, desc_embed=False, version="mimic3"
):
    """
    Inputs:
    ...
    """
    ind2w, w2ind, ind2c, c2ind, dv_dict = (
        dicts["ind2w"],
        dicts["w2ind"],
        dicts["ind2c"],
        dicts["c2ind"],
...
def load_vocab_dict(args, vocab_file):
    ...
    if (
            args.public_model
            and args.Y == "full"
            and args.version == "mimic3"
            and args.model == "conv_attn"
    ):
...
def load_lookups(args, desc_embed=False, diagnoses_code_file=None, procedures_code_file=None,
                 icd9_descriptions_file=None):
    """
    Inputs:
    ...
    if args.Y == "full":
        ind2c, desc_dict = load_full_codes(args.data_path, version=args.version,
                                           diagnoses_code_file=diagnoses_code_file,
                                           procedures_code_file=procedures_code_file,
                                           icd9_descriptions_file=icd9_descriptions_file
                                           )
    else:
        codes = set()
...
def load_full_codes(train_path, version="mimic3", diagnoses_code_file=None, procedures_code_file=None,
                    icd9_descriptions_file=None):
    """
    Inputs:
    ...
    desc_dict = load_code_descriptions(version=version, diagnoses_code_file=diagnoses_code_file,
                                       procedures_code_file=procedures_code_file,
                                       icd9_descriptions_file=icd9_descriptions_file)
...
def load_code_descriptions(version="mimic3", diagnoses_code_file=None, procedures_code_file=None,
                           icd9_descriptions_file=None):
    ...
    if version == "mimic2":
        ...
        diagnoses_code_file = f"{DATA_DIR}/D_ICD_DIAGNOSES.csv" if diagnoses_code_file is None else diagnoses_code_file
        procedures_code_file = f"{DATA_DIR}/D_ICD_PROCEDURES.csv" if procedures_code_file is None else procedures_code_file
        icd9_descriptions_file = f"{DATA_DIR}/ICD9_descriptions" if icd9_descriptions_file is None else icd9_descriptions_file
        with open(diagnoses_code_file, "r") as descfile:
        ...
        with open(procedures_code_file, "r") as descfile:
            r = csv.reader(descfile)
            # header
            next(r)
            for row in r:
        ...
        with open(icd9_descriptions_file, "r") as labelfile:
