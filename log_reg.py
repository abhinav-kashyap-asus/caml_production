...
def main(Y, train_fname, dev_fname, vocab_file, version, n):
    ...
    data_path = (
        train_fname.replace("_bows", "") if "_bows" in train_fname else train_fname
    )
    dicts = datasets.load_lookups(
    ...
    clf = OneVsRestClassifier(
        LogisticRegression(C=C, max_iter=MAX_ITER, solver="sag"), n_jobs=-1
    )
    ...
    # evaluate
    metrics, fpr, tpr = evaluation.all_metrics(
        yhat_full, yy_dv, k=[8, 15], yhat_raw=yhat_full_raw
    )
    ...
    model_dir = os.path.join(
        MODEL_DIR, "_".join(["log_reg", time.strftime("%b_%d_%H:%M", time.localtime())])
    )
    os.mkdir(model_dir)
        ...
        for i, row in tqdm(enumerate(reader)):
            ...
            for label in labels:
                ...
                w = [word for word in text.split()][
                    fourgrams_scores.index(
                        max(fourgrams_scores)
    ...
    if len(sys.argv) < 8:
        print(
            ...
            + str(
                ...
                + " [|Y|] [train_dataset] [dev_dataset] [vocab_file] [version] [size of ngrams (0 if do not wish to generate)]"
            )
        )
        sys.exit(0)
    main(
