...
class BaseModel(nn.Module):
    def __init__(
        self, Y, embed_file, dicts, lmbda=0, dropout=0.5, gpu=True, embed_size=100
    ):
...
class BOWPool(BaseModel):
    ...
    def __init__(
        ...
        super(BOWPool, self).__init__(
            Y, embed_file, dicts, lmbda, dropout=dropout, gpu=gpu, embed_size=embed_size
        )
    ...
    def forward(self, x, target, desc_data=None, get_attention=False):
        ...
        if get_attention:
            ...
            x, argmax = F.max_pool1d(
                self.tanh(c), kernel_size=c.size()[2], return_indices=True
            )
...
class VanillaRNN(BaseModel):
    ...
    def __init__(
        ...
        super(VanillaRNN, self).__init__(
            Y, embed_file, dicts, embed_size=embed_size, gpu=gpu
        )
        self.gpu = gpu
