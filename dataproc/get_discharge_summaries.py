...
def write_discharge_summaries(out_file, preprocessing=True):
        ...
        with open(out_file, "w") as outfile:
            ...
            for line in tqdm(notereader):
                ...
                if category == "Discharge summary":
                    ...
                    if preprocessing:
                        tokens = [
                            t.lower() for t in tokenizer.tokenize(note) if not t.isnumeric()
                        ]
                    else:
                        tokens = [t.lower() for t in note.split()]

                    text = " ".join(tokens)
                    csv_writer.writerow([line[1], line[2], line[4], text])
                i += 1
    return out_file
